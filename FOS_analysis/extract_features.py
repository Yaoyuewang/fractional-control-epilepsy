...
for patient in patients: 
    for seizure in range (1, 9):
        if os.path.exists(f'{main_pathname}/{patient}/ictal-block-{seizure}_parameters_{window_size}sec_1iter.mat'):
            ...
            ictal_end = min(num_windows-20, ictal_start + 20)
            
            ictal_alpha_all = ictal_data['alpha']
            ictal_eigen_all = ictal_data['eigenvalues']

            interictal_alpha = interictal_data['alpha'][:, 0:20]
            interictal_eigen = interictal_data['eigenvalues'][:, 0:20]

            preictal_alpha = ictal_alpha_all[:, 0:ictal_start]
            ictal_alpha = ictal_alpha_all[:, ictal_start:ictal_end]
            postictal_alpha = ictal_alpha_all[:, num_windows - 20:num_windows]
            ...
            np.savez(f'{main_pathname}/{patient}/fos_features_block_{seizure}.npz',
                ...
                timepoints = list(range(num_windows-20,num_windows)))
